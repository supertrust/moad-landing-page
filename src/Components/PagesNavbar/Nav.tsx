...
const Nav = ({ page }: Props) => {
  ...
  const getClassName = (href: string) => {
    const baseClass =
      'h-16 px-5 py-5 border border-gray-300 text-gray-600  justify-center items-center flex';

    if (pathname === `/${page}${href}`) {
      return `${baseClass} bg-daisyBush-60 text-white font-bold`;
    } else {
      return baseClass;
    }
  };
